...
# Model-based methods use machine learning techniques to predict ratings. 
# Singular Value Decomposition (SVD) is a matrix factorization method that decomposes the user-item matrix into lower-dimensional matrices representing latent "factors" for users and items.
...
svd = SVD(n_factors=50, n_epochs=100, random_state=42) # n_factors is the number of latent features
svd.fit(trainset)
...
print(svd_recs)
print(df[df["userId"] == 1].sort_values("rating"))

# %% [markdown]
# ## 5. Visualize: Movie Latent Factors from SVD
# The SVD model learns a set of latent factors for each movie. 
# We can think of these factors as capturing underlying characteristics like "action-ness," "comedy-ness," or "for-kids-ness." 
# By reducing these factors to 2D using PCA or t-SNE, we can visualize the relationships between movies in a "taste space."
...
# Map inner movie IDs to raw movie IDs for SVD visualization
trainset_inner_to_raw_iids = {inner_id: trainset.to_raw_iid(inner_id) for inner_id in range(trainset.n_items)}
...
# %%
# Might be even nicer if we use movie ratings than SVD for visualization
reducer = TSNE(n_components=2, random_state=42, perplexity=30)
movie_factors_2d = reducer.fit_transform(user_item_matrix_sparse.T.toarray())
factor_df = pd.DataFrame(movie_factors_2d, columns=['x', 'y'])
factor_df['title'] = user_item_matrix.columns
factor_df = pd.merge(factor_df, movies_df, on='title')
...
fig = px.scatter(
    factor_df.sample(2000, random_state=42),  # Plot a sample for performance
    x='x', y='y',
    hover_name='title',
    hover_data=['genres'],
    title=f'2D Representation of Movies based on SVD Latent Factors ({use_dim_red.upper()})'
)
...
fig.update_layout(
    ...
    title={
        'text': f"2D Movie 'Taste Space' from SVD ({use_dim_red.upper()})",
        'y': 0.95,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top'
